...
SELECT
  ...
  CAST(A.ACCOUNTTERMINATIONDATE AS DATE) AS ACCOUNT_TEMINATION_DT,
  ...
  'NULL' AS ATTRIBUTE4,
  'NULL' AS ATTRIBUTE5,
  CAST(S.CUSTOMERACCOUNTSITEUSECREATIONDATE AS DATE) AS  CREATION_DT  ,
  CAST(S.CUSTOMERACCOUNTSITEUSELASTUPDATEDATE AS DATE) AS LAST_UPDATE_DT,
  ...
  'Y' AS ACTIVE_FLAG,
  'N' AS DELETE_FLAG,
  CAST(CURRENT_TIMESTAMP() AS DATETIME) AS W_INSERT_DT,
  CAST(CURRENT_TIMESTAMP() AS DATETIME) AS W_UPDATE_DT,
  'NULL' AS PROSPECT_FLAG,
...
WHERE
  ...
  AND CAST(S.CUSTOMERACCOUNTSITEUSELASTUPDATEDATE AS STRING) >= CAST({{ last_load_date }} AS STRING)
 
 --AND CAST(S.CUSTOMERACCOUNTSITEUSELASTUPDATEDATE AS STRING FORMAT 'YYYY-MM-DD')>= CAST( {{last_load_date}}  as STRING)
)
